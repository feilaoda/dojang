...
class BaseQuery(orm.Query):
    ...
    def limits(self, offset, count=30):
        return self.offset(offset).limit(count)
    ...
    _underscore_operators = {
        ...
        'notnull': lambda c, x:x and c != None,
...
class SQLAlchemy(object):
    ...
    @classmethod
    def create_sqlalchemy(cls, engine_url, instance_name, kwargs=None):
        """create single instance SQLAlchemy"""
        if hasattr(cls, instance_name):
            return getattr(cls, instance_name)
        if kwargs is None:
            kwargs = {}
        _instance = cls(engine_url, **kwargs)
